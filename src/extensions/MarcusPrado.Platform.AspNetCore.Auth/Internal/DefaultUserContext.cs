...
internal sealed class DefaultUserContext : IUserContext
{
    ...
    /// <inheritdoc />
    public IReadOnlyCollection<string> Scopes { get; private set; } = Array.Empty<string>();

    /// <inheritdoc />
    public string? GetClaim(string claimType) => _principal?.FindFirstValue(claimType);
    ...
    public void SetUser(ClaimsPrincipal principal)
    {
        ...
        var scopeClaim = principal.FindFirstValue("scope") ?? principal.FindFirstValue("scp") ?? string.Empty;
