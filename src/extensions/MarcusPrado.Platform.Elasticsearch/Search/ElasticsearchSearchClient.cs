...
public sealed class ElasticsearchSearchClient : ISearchClient, IIndexManager
{
    ...
    /// <inheritdoc />
    public async Task<SearchResult<T>> SearchAsync<T>(AbsSearch.SearchQuery query, CancellationToken ct = default)
    {
        ...
        var response = await _client
            .SearchAsync<T>(
                s =>
                {
                    s.Index(query.IndexName).From(query.Skip).Size(query.Take);

                    if (filterClauses.Count > 0)
                    {
                        s.Query(q =>
                            q.Bool(b => b.Must(m => m.QueryString(qs => qs.Query(query.Query))).Filter(filterClauses))
                        );
                    }
                    else
                    {
                    ...
                    if (query.SortField is not null)
                    {
                        s.Sort(so =>
                            so.Field(
                                ...
                                f =>
                                {
                                    f.Order(query.SortDescending ? SortOrder.Desc : SortOrder.Asc);
                                }
                            )
                        );
                    }
    ...
    /// <inheritdoc />
    public async Task<T?> GetByIdAsync<T>(string indexName, string id, CancellationToken ct = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(indexName);
        ArgumentException.ThrowIfNullOrWhiteSpace(id);

        var response = await _client.GetAsync<T>(id, g => g.Index(indexName), ct).ConfigureAwait(false);
    ...
    public async Task CreateIndexAsync(string indexName, CancellationToken ct = default)
    {
        ...
        var response = await _client.Indices.CreateAsync(indexName, ct).ConfigureAwait(false);
    ...
    public async Task DeleteIndexAsync(string indexName, CancellationToken ct = default)
    {
        ...
        var response = await _client.Indices.DeleteAsync(indexName, ct).ConfigureAwait(false);
    ...
    /// <inheritdoc />
    public async Task IndexDocumentAsync<T>(string indexName, string id, T document, CancellationToken ct = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(indexName);
        ArgumentException.ThrowIfNullOrWhiteSpace(id);
    ...
    public async Task DeleteDocumentAsync(string indexName, string id, CancellationToken ct = default)
    {
        ...
        var response = await _client.DeleteAsync<object>(id, d => d.Index(indexName), ct).ConfigureAwait(false);

        if (!response.IsValidResponse)
            throw new InvalidOperationException(response.DebugInformation);
    }

    private static List<Query> BuildFilterClauses(IReadOnlyDictionary<string, string>? filters)
    {
