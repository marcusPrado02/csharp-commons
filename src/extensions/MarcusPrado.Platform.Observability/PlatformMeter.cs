...
public sealed class PlatformMeter : IDisposable
{
    ...
    public PlatformMeter(string serviceName)
    {
        ...
        RequestCount = _meter.CreateCounter<long>("platform.requests.total", "requests", "Total HTTP requests processed");

        RequestDuration = _meter.CreateHistogram<double>("platform.request.duration_ms", "ms", "HTTP request duration in milliseconds");

        ActiveRequests = _meter.CreateUpDownCounter<int>("platform.requests.active", "requests", "Currently active requests");
