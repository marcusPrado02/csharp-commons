...
#pragma warning restore S3011
    ...
    public async Task CreateIndexAsync(string indexName, CancellationToken ct = default)
    {
        ...
        var response = await _client.Indices
            .CreateAsync(new CreateIndexRequest(indexName), ct).ConfigureAwait(false);
    ...
    public async Task DeleteIndexAsync(string indexName, CancellationToken ct = default)
    {
        ...
        var response = await _client.Indices
            .DeleteAsync(new DeleteIndexRequest(indexName), ct).ConfigureAwait(false);
    ...
    public async Task<bool> IndexExistsAsync(string indexName, CancellationToken ct = default)
    {
        ...
        var response = await _client.Indices
            .ExistsAsync(new IndexExistsRequest(indexName), ct).ConfigureAwait(false);

        return response.Exists;
    }

    /// <inheritdoc />
    public Task IndexDocumentAsync<T>(
        ...
        string indexName, string id, CancellationToken ct = default)
    {
        ...
        var response = await _client.DeleteAsync(
            new DeleteRequest(indexName, id), ct).ConfigureAwait(false);
    ...
    private async Task<SearchResult<T>> SearchCoreAsync<T>(
        AbsSearch.SearchQuery query, CancellationToken ct)
        where T : class
    {
        var response = await _client.SearchAsync<T>(s =>
        {
            s = s.Index(query.IndexName)
                 .From(query.Skip)
                 .Size(query.Take);
            ...
            {
                s = s.Sort(so => query.SortDescending
                    ? so.Descending(query.SortField)
                    : so.Ascending(query.SortField));
            }
    ...
    private async Task<T?> GetByIdCoreAsync<T>(
        string indexName, string id, CancellationToken ct)
        where T : class
    {
        var response = await _client.GetAsync<T>(
            new GetRequest(indexName, id), ct).ConfigureAwait(false);

        return response.Found ? response.Source : default;
    }

    private async Task IndexDocumentCoreAsync<T>(
        string indexName, string id, T document, CancellationToken ct)
        where T : class
    {
