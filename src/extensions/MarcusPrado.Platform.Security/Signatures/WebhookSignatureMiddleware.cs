...
public sealed class WebhookSignatureMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        // 1. Read timestamp
        if (
            !context.Request.Headers.TryGetValue(_options.TimestampHeader, out var tsHeader)
        ...
        var expected = Convert
            .ToHexString(hmac.ComputeHash(System.Text.Encoding.UTF8.GetBytes(payload)))
            .ToLowerInvariant();

        // 4. Compare
        if (
            !context.Request.Headers.TryGetValue(_options.HeaderName, out var sigHeader)
