    ...
    where THub : Hub
{
    ...
    public HubRealtimePublisher(IHubContext<THub> hubContext)
    ...
    /// <inheritdoc />
    public Task PublishToTenantAsync<T>(string tenantId, string topic, T payload, CancellationToken cancellationToken = default)
        => _hubContext.Clients.Group($"tenant:{tenantId}").SendAsync(topic, payload, cancellationToken);

    /// <inheritdoc />
    public Task PublishToUserAsync<T>(string userId, string topic, T payload, CancellationToken cancellationToken = default)
