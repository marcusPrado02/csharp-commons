...
public sealed class ContractCompatibilityRules
{
    ...
    public void EventContracts_ShouldImplement_IEventContract()
    {
        ...
        var violations = KnownAssemblies.Contracts
            .GetExportedTypes()
            .Where(t =>
                t.IsClass &&
                !t.IsAbstract &&
    ...
    public void ApiContractTypes_ShouldNotBeSealed()
    {
        ...
        violations.Should().BeEmpty(
            because: "[ApiContract] classes must not be sealed; minor-version "
            + "additions extend via inheritance");
    }
    ...
    public void Contracts_ShouldNotHaveDependencyOnInfrastructure()
    {
        ...
        })
        {
            ...
            result.IsSuccessful.Should().BeTrue(
                because: $"Contracts must not reference {forbidden}. Failing: [{failing}]");
        }
    }
    ...
    private static bool ContainsContractsSegment(string namespaceName) =>
        namespaceName.Split('.')
            .Any(seg => seg.Equals("Contracts", StringComparison.Ordinal));
}
