...
public sealed class ResultAsyncExtensionsTests
{
    ...
    {
        var result = await Ok(5).BindAsync(x =>
            Task.FromResult<Result<string>>(x.ToString()));
    ...
    public async Task BindAsync_OnSuccess_NextFails_ReturnsThatFailure()
    {
        ...
        var result = await Ok(-1).BindAsync(_ =>
            Task.FromResult<Result<string>>(innerErr));
    ...
    {
        var invoked = false;
        var result = await Fail<int>(SomeError).BindAsync(x =>
        {
            invoked = true;
    ...
    {
        var captured = 0;
        var result = await Ok(42).OnSuccessAsync(v => { captured = v; return Task.CompletedTask; });
    ...
    {
        var invoked = false;
        await Fail<int>(SomeError).OnSuccessAsync(_ => { invoked = true; return Task.CompletedTask; });
    ...
    public async Task OnFailureAsync_OnFailure_InvokesAction_ReturnsOriginal()
    {
        ...
        var result = await Fail<int>(SomeError)
            .OnFailureAsync(e => { captured = e; return Task.CompletedTask; });
    ...
    {
        var invoked = false;
        await Ok(1).OnFailureAsync(_ => { invoked = true; return Task.CompletedTask; });
    ...
    public async Task EnsureAsync_AlreadyFailed_PassesThroughOriginalError()
    {
        ...
        var result = await Fail<int>(SomeError)
            .EnsureAsync(_ => Task.FromResult(true), otherErr);
